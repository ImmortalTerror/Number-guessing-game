...
print("Im thinking of a number between " + colored("0", "red") +" and " + colored("100", "red") + """
You need to guess this number
If I tell you """ + colored("boiling", "red", attrs=["bold"]) + ", your within " + colored("5", "red") + """
If I tell you """ + colored("hot", "yellow", attrs=["bold"]) + ", your within " + colored("10", "yellow") + """
If I tell you """ + colored("warm", "green", attrs=["bold"]) + ", your within " + colored("30", "green") + """
If I tell you """ + colored("cold", "cyan", attrs=["bold"]) + ", your within " + colored("40", "cyan") + """
And if I say """ + colored("freezing", "blue", attrs=["bold"]) + ", your over " + colored("40", "blue") + " away from my number\n"
)
...
while playing == True:
    ...
    while guessing == True:
        ...
        print("This is your " + colored("first", "red", attrs=["bold"]) +" guess" if guesses == 1 else "You have tried " + colored(guesses, "red", attrs=["bold"]) +" times")
        ...
        try:
            ...
            elif guess in range(secret-10, secret-5) or guess in range(secret+5, secret+10):
                cprint("Hot\n", "yellow", attrs=["bold"])

            # Checks if the number is within 30 of the secret number
            elif guess in range(secret-20, secret-10) or guess in range(secret+10, secret+20):
                cprint("Warm\n", "green", attrs=["bold"])
            
            # Checks if the number is within 40 of the secret number
            elif guess in range(secret-40, secret-20) or guess in range(secret+20, secret+40):
                cprint("Cold\n", "cyan", attrs=["bold"])
            
            # Checks if the number is over 40 away from the secret number
            elif guess <= secret+40 and guess != secret or guess >= secret-40 and guess != secret:
                cprint("Freezing\n", "blue", attrs=["bold"])
            
            # Checks if the number is the secret number
            elif guess == secret: guessing = False
    ...
    print("You guessed the right number in " + colored(guesses, "red", attrs=["bold"]) + " guesses!")

    # This is if the player wants to play again
    win = True
    while win == True:
        print("Would you like to play again? " + colored("(Y/N)", "green", attrs=["bold"]))
